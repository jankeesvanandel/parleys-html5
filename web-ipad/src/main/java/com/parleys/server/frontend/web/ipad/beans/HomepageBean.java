...
public class HomepageBean extends AbstractParleysBean {
    ...
    public void init() {
        ...
        final List<? extends AbstractDTO> thumbnailsIn = getParleysService().getFeatured(FeaturedType.PRESENTATION);
        transformToThumbnails(thumbnailsIn);

        photoSlideShow = getParleysService().loadPhotoSlideShow();
    }

    public String viewThumbnails(final Filter filter, final Filter.Type filterType, final int index) {
    ...
    public String showMoreThumbnails(final Filter filter, final Filter.Type filterType, final int amount) {
        getPagingBean().setPaginatedList(Collections.<Thumbnail>emptyList());
    ...
    private void transformToThumbnails(final List<? extends AbstractDTO> thumbnailsIn) {
        ...
        for (int i = 0, thumbnailsInSize = thumbnailsIn.size(); i < thumbnailsInSize; i++) {
            final AbstractDTO thumbnailDto = thumbnailsIn.get(i);
...
//            thumbnail.setPhoto(photo);
    ...
    private Thumbnail createThumbnailFromDto(final AbstractDTO abstractDTO) {
        ...
        if (abstractDTO instanceof PresentationOverviewDTO) {
            ...
            final String url = ((PresentationOverviewDTO) abstractDTO).getThumbnailURL();
            ...
            final String url = ((SpaceOverviewDTO) abstractDTO).getThumbnailURL();
            ...
            final String url = ((ChannelOverviewDTO) abstractDTO).getThumbnailURL();
    ...
    public void setHomepageViewBean(final HomepageViewBean homepageViewBean) {
